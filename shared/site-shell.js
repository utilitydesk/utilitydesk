...
(function () {
  ...
  function recoverLegacyMarkup() {
    ...
    nodes.forEach(textNode => {
      ...
      if (match) {
        ...
        textNode.remove();
        // The original parser discarded the matching </section>. Re-parent the
        // intended section contents until the next real <section> boundary.
        while (sibling) {
  ...
  function ensurePdfInventory() {
    ...
    if (!section) return;
    const grid = section.querySelector('.tools-grid'); if (!grid) return;
    ...
    PDF_TOOLS.forEach(([slug,title,desc,icon]) => {
      const href = '/pdf-tools/' + slug + '/'; if (existing.has(href)) return;
      const a = document.createElement('a'); a.href = href; a.className = 'tool-card';
      a.dataset.title = title.toLowerCase(); a.dataset.desc = desc.toLowerCase();
    ...
    document.querySelectorAll('.category-block .cat-count').forEach(el => {
      const block = el.closest('.category-block'); if (block) el.textContent = block.querySelectorAll('.tool-card').length;
    });
  ...
  function ensureDocumentInventory() {
    ...
    const grid = hrBlock && hrBlock.querySelector('.tools-grid'); if (!grid) return;
    ...
    DOC_TOOLS.forEach(([slug,title,desc,icon]) => {
      const href = '/document-generators/' + slug + '/'; if (existing.has(href)) return;
      const a = document.createElement('a'); a.href = href; a.className = 'tool-card';
      a.dataset.title = title.toLowerCase(); a.dataset.desc = desc.toLowerCase();
      a.innerHTML = '<div class="tool-icon">' + icon + '</div><div class="tool-card-body"><h3>' + title + '</h3><p>' + desc + '</p></div>';
      grid.appendChild(a);
  ...
  function normalizeInventoryCounts() {
    ...
    if (path === '/hr/') {
      const count = document.querySelector('#searchCount'); if (count) count.textContent = '23 tools available';
      const input = document.querySelector('#searchInput'); if (input) input.placeholder = 'Search 23+ tools...';
    }
  }
